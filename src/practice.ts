...
document.addEventListener("DOMContentLoaded", () => {
	...
	const gridParts = Array.from(
		document.querySelectorAll(".grid-part"),
	) as HTMLElement[];
	const rewardImage = document.getElementById(
		"reward-image",
	) as HTMLImageElement;
	const nextRoundBtn = document.getElementById(
		"next-round-btn",
	) as HTMLButtonElement;
	const feedbackSummary = document.getElementById(
		"feedback-summary",
	) as HTMLElement;
	...
	const endRoundBtn = document.getElementById(
		"end-round-btn",
	) as HTMLButtonElement;
	...
	function hasOptions(
		problem: Problem,
	): problem is Problem & { options: { label: string; value: number }[] } {
	...
	function chooseRandomRewardImage() {
		...
		let completedImages: number[] = JSON.parse(
			localStorage.getItem("completedRewardImages") || "[]",
		);

		// Find available images
		let availableImages = Array.from(
			{ length: numberOfRewardImages },
			(_, i) => i + 1,
		...
		if (availableImages.length === 0) {
			...
			localStorage.setItem(
				"completedRewardImages",
				JSON.stringify(completedImages),
			);
		}
		...
		if (isCorrect) {
			...
			if (isPictureComplete()) {
				...
				if (currentRewardImageId !== null) {
					const completedImages: number[] = JSON.parse(
						localStorage.getItem("completedRewardImages") || "[]",
					);
